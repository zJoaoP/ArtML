...
def get_first_n_index_of(predictions: list, node: int, size=9):
    return [i for i, n in enumerate(predictions) if n == node][:size]
...
if __name__ == '__main__':
    ...
    names, images = load_images_from(args.dataset, 30000)
    ...
    for sample in encodings:
        predictions.append(som_model.predict(sample))
